#!/usr/bin/env python3
"""Produit des cartes WebP depuis la grille native AROME (France ou Outre-Mer).

Les champs ne sont jamais reconstruits depuis les communes. Les points natifs
du GRIB sont reprojetés sur une image Web Mercator couvrant le domaine
demandé, puis les côtes, frontières nationales et éventuelles limites
administratives (départements, ou tout autre découpage fourni par l'appelant)
...
class AromeMapRenderer:
    ...
    def __init__(
        ...
        *,
        ...
        boundary_latitudes: np.ndarray | None = None,
        boundary_longitudes: np.ndarray | None = None,
        boundary_codes: Sequence[str] | None = None,
        ...
        self.boundary_latitudes = (
            np.asarray(boundary_latitudes, dtype=np.float64)
            if boundary_latitudes is not None
            else None
        )
        self.boundary_longitudes = (
            np.asarray(boundary_longitudes, dtype=np.float64)
            if boundary_longitudes is not None
            else None
        )
        self.boundary_codes = (
            list(boundary_codes) if boundary_codes is not None else None
        )
    ...
    def _department_svg_path(self) -> str:
        if (
            self.boundary_latitudes is None
            or self.boundary_longitudes is None
            or self.boundary_codes is None
            or len(self.boundary_codes) != len(self.boundary_latitudes)
        ):
            return ""

        source = np.column_stack(
            (
                self.boundary_longitudes * self._longitude_scale,
                self.boundary_latitudes,
            )
        )
        target = np.column_stack(
        ...
        codes = {
            code: index + 1
            for index, code in enumerate(sorted(set(self.boundary_codes)))
        }
        encoded = np.asarray(
            [codes.get(code, 0) for code in self.boundary_codes]
        )
        departments = encoded[indexes].reshape(self.height, self.width)
        within_domain = distances.reshape(self.height, self.width) <= 0.18
